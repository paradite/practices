...
var findWords = function (board, words) {
  ...
  const ans = [];
  function dfs(word, wordIndex, x, y, visited) {
    ...
    visited[`${x}-${y}`] = true;
    if (dfs(word, wordIndex + 1, x - 1, y, visited)) {
      return true;
    }
    if (dfs(word, wordIndex + 1, x + 1, y, visited)) {
      return true;
    }
    if (dfs(word, wordIndex + 1, x, y + 1, visited)) {
      return true;
    }
    if (dfs(word, wordIndex + 1, x, y - 1, visited)) {
      return true;
  ...
  for (let i = 0; i < words.length; i++) {
    const word = words[i];
    const startingPos = letterMap[word[0]];
    if (startingPos) {
      for (let j = 0; j < startingPos.length; j++) {
        const [sx, sy] = startingPos[j];
        const res = dfs(word, 0, sx, sy, {});
        if (res) {
          ans.push(word);
          break;
        }
