...
var findWords = function (board, words) {
  const allPrefixMap = {};
  const allPrefixMapReverse = {};
  for (let i = 0; i < words.length; i++) {
    const word = words[i];
    const wordReverse = words[i].split('').reverse().join('');
    for (let j = 0; j < word.length; j++) {
      allPrefixMap[word.slice(0, j + 1)] = true;
      allPrefixMapReverse[wordReverse.slice(0, j + 1)] = true;
    }
  }
  // console.log('TCL ~ allPrefixMap', allPrefixMap);
  // console.log('TCL ~ allPrefixMap', Object.entries(allPrefixMap).length);
  // console.log('TCL ~ allPrefixMapReverse', allPrefixMapReverse);
  // console.log(
  //   'TCL ~ allPrefixMapReverse',
  //   Object.entries(allPrefixMapReverse).length
  // );

  const finalPrefixMap =
    Object.entries(allPrefixMapReverse).length >
    Object.entries(allPrefixMap).length
      ? allPrefixMap
      : allPrefixMapReverse;
  // console.log('TCL ~ finalPrefixMap', Object.entries(finalPrefixMap).length);

  const validPrefixMap = {};

  const ans = [];
  // let dsfCount = 0;
  function dfs(currWord, x, y, visited) {
    ...
    const newPrefix = currWord + currChar;
    if (!finalPrefixMap[newPrefix]) {
      // not what we are looking for
      return;
    }
    // valid prefix
    // console.log('TCL ~ newPrefix', newPrefix);
    validPrefixMap[newPrefix] = true;
    visited[x * 10 + y] = true;
    if (dfs(newPrefix, x - 1, y, visited)) {
      return true;
    }
    if (dfs(newPrefix, x + 1, y, visited)) {
      return true;
    }
    if (dfs(newPrefix, x, y + 1, visited)) {
      return true;
    }
    if (dfs(newPrefix, x, y - 1, visited)) {
      return true;
  ...
  for (let i = 0; i < words.length; i++) {
    const word = words[i];
    const wordReverse = words[i].split('').reverse().join('');
    if (validPrefixMap[word] || validPrefixMap[wordReverse]) {
      ans.push(word);
    }
  }
  // console.log('TCL ~ visitedCount', visitedCount);
  // console.log({ dsfCount });
