...
var maxProbability = function (n, edges, succProb, start, end) {
  ...
  for (let i = 0; i < edges.length; i++) {
    ...
    neighbour[a].push(b);
    neighbour[b].push(a);
    distMap[`${a}-${b}`] = succProb[i];
    distMap[`${b}-${a}`] = succProb[i];
  }
  let next = queue.pop();
  // console.log('maxProbability -> next', next);
  while (next) {
    // console.log('maxProbability -> next', next);
    for (let i = 0; i < neighbour[next.id].length; i++) {
      const element = neighbour[next.id][i];
      if (!visited[element]) {
        // prettier-ignore
        // console.log("maxProbability -> distMap[`${next}-${element}`]", distMap[`${next.id}-${element}`])
        const alt = dist[next.id] * distMap[`${next.id}-${element}`];
        if (alt > dist[element]) {
          dist[element] = alt;
          queue.decreaseKey(element, alt);
        }
      }
    }
    // console.log('maxProbability -> dist', dist);
    visited[next] = true;
    if (queue.size() > 0) {
      next = queue.pop();
    } else {
